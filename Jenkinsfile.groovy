...
"""
    stage('Build Docker Image') {
                steps {
                    withCredentials([string(credentialsId: '5f567787-4fc1-47c4-8194-d2885f1d1d08', variable: 'AWS_ECR_URL')]) {
                    script {
        ...
        steps {
                    withCredentials([string(credentialsId: '5f567787-4fc1-47c4-8194-d2885f1d1d08', variable: 'AWS_ECR_URL')]) {
                    withAWS(region: "${AWS_ECR_REGION}", credentials: 'aws') {
                    script {
                        def login = ecrLogin()
                        sh('#!/bin/sh -e\n' + "${login}") // hide logging
    ...
    stage('Deploy in ECS') {
        steps {
                    withCredentials([string(credentialsId: '5f567787-4fc1-47c4-8194-d2885f1d1d08', variable: 'AWS_ECS_EXECUTION_ROL')]) {
                    script {
